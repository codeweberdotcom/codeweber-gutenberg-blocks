...
(function () {
	...
	window.reinitScrollCue = function () {
		if (
			typeof window.scrollCue !== 'undefined' &&
			typeof window.scrollCue.update === 'function'
		) {
			try {
	...
	const retryInterval = setInterval(function () {
		...
		if (
			...
			console.log(
				'✅ scrollCue detected after ' + attempts + ' attempts'
			);
			clearInterval(retryInterval);
		} else if (attempts >= maxAttempts) {
			console.warn(
				'⚠️ scrollCue not detected after ' +
					maxAttempts +
					' attempts. Animation preview may not work.'
			);
			clearInterval(retryInterval);
		}
	}, 2000);
