...
export const LoadMoreControl = ({
	...
	const textOptions = [
		{
			label: __('Show More', 'codeweber-gutenberg-blocks'),
			value: 'show-more',
		},
		{
			label: __('Load More', 'codeweber-gutenberg-blocks'),
			value: 'load-more',
		},
		{
			label: __('Show More Items', 'codeweber-gutenberg-blocks'),
			value: 'show-more-items',
		},
		{
			label: __('More Posts', 'codeweber-gutenberg-blocks'),
			value: 'more-posts',
		},
		{
			label: __('View All', 'codeweber-gutenberg-blocks'),
			value: 'view-all',
		},
		{
			label: __('Show All', 'codeweber-gutenberg-blocks'),
			value: 'show-all',
		},
	];
	...
	return (
		<>
			<RangeControl
				...
				value={initialCount}
				onChange={(value) =>
					setAttributes({ [`${attributePrefix}InitialCount`]: value })
				}
				min={1}
				max={50}
				help={__(
			...
			<ToggleControl
				...
				onChange={(value) =>
					setAttributes({ [`${attributePrefix}Enable`]: value })
				}
			/>
			...
			{enableLoadMore && (
				<>
					<SelectControl
						...
						options={typeOptions}
						onChange={(value) =>
							setAttributes({ [`${attributePrefix}Type`]: value })
						}
						help={__(
							'Select element type: button or link',
							'codeweber-gutenberg-blocks'
						)}
					/>
					...
					<SelectControl
						...
						options={textOptions}
						onChange={(value) =>
							setAttributes({ [`${attributePrefix}Text`]: value })
						}
						help={__(
							'Select text for button or link',
							'codeweber-gutenberg-blocks'
						)}
					/>
					...
					<RangeControl
						...
						onChange={(value) =>
							setAttributes({
								[`${attributePrefix}LoadMoreCount`]: value,
							})
						}
						min={1}
						max={50}
						help={__(
							'Number of items loaded when clicking the button',
							'codeweber-gutenberg-blocks'
						)}
					/>
