...
export const PostGridItemRender = ({
	...
	if (postType === 'testimonials') {
		...
		const testimonialText =
			post.text || post.meta?._testimonial_text || postDescription || '';
		const authorName =
			post.authorName || post.meta?._testimonial_author_name || '';
		const authorRole =
			post.authorRole || post.meta?._testimonial_author_role || '';
		const company = post.company || post.meta?._testimonial_company || '';
		const rating =
			post.rating || parseInt(post.meta?._testimonial_rating || '5');
		const ratingClass =
			post.ratingClass ||
					...
					{enableLink ? (
						...
						>
							<div className={`position-relative`}>
								<div
									className={`shape rounded ${bgColor} rellax d-md-block`}
									style={{
										bottom: '-0.75rem',
										right: '-0.75rem',
										width: '98%',
								...
								></div>
								<div
									className={`card h-100 lift${cardRadius ? ' ' + cardRadius : ''}`}
								>
									{imageUrl && (
										<figure
											className={`card-img-top${cardRadius ? ' ' + cardRadius : ''}`}
										>
											<img
	...
	if (postType === 'offices') {
		const cardRadius = borderRadius || '';
		const cardRadiusClass = cardRadius ? ' ' + cardRadius : '';
		const figureRadius = cardRadius && cardRadius !== 'rounded-0' ? ' rounded-start' : (cardRadius ? ' ' + cardRadius : '');

		if (template === 'list-sm') {
			return (
				<div className={`card card-horizontal card-horizontal-always${cardRadiusClass}`}>
					<figure className={`card-img position-relative text-reset${figureRadius}`}>
						<a href={isEditor ? '#' : postLink} onClick={isEditor ? (e) => e.preventDefault() : undefined}>
							<img src={imageUrl} alt={postTitle} className="w-100 h-100 object-fit-cover" loading="lazy" />
						</a>
					</figure>
		...
		if (template === 'list-sm-text') {
			return (
				<div className={`card${cardRadiusClass}`}>
					<div className="card-body p-5 d-flex flex-column">
						<h4 className="post-title mb-2">
							<a href={isEditor ? '#' : postLink} className="link-dark" onClick={isEditor ? (e) => e.preventDefault() : undefined}>
								{titleLimited}
	...
	if (template === 'card') {
		...
		return (
			<article className="h-100">
				<a
					href={isEditor ? '#' : postLink}
					className="card-link d-block text-decoration-none d-flex flex-column h-100 lift"
		...
		const documentFileName = documentFileUrl
			? documentFileUrl.split('/').pop()
			: '';
