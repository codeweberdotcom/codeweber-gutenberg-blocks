...
class Plugin {
	...
	public static function get_contacts_callback($request) {
		try {
			...
			// Получаем все опции Redux за один вызов
			$all_options = get_option($opt_name, []);

			// Вспомогательная функция для получения адреса
			$get_contact_address = function($addressType) use ($all_options) {
				try {
					$prefix = $addressType === 'legal' ? 'juri' : 'fact';
					$address_parts = [];
					foreach (['postal', 'country', 'region', 'city', 'street', 'house', 'office'] as $part) {
						$value = $all_options["{$prefix}-{$part}"] ?? '';
						if (!empty($value)) {
							$address_parts[] = $value;
						}
					}
			...
			$legal_address = $get_contact_address('legal');
			$actual_address = $get_contact_address('actual');
			$email = $all_options['e-mail'] ?? '';
			...
			foreach ($phone_keys as $phone_key) {
				try {
					$phone_value = $all_options[$phone_key] ?? '';
