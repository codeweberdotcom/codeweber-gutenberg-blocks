...
(function ($) {
	...
	let hotspotEditor = {
		...
		bindEvents: function () {
			...
			this.annotationBox.on(
				'click',
				'.cw-hotspot-main-image',
				function (e) {
		...
		renderHotspot: function (point) {
			...
			this.annotationBox
				.find('.cw-hotspot-point[data-point-id="' + point.id + '"]')
				.remove();
			...
			pointHtml.draggable({
				...
				stop: function (event, ui) {
					...
					const annotationRect =
						self.annotationBox[0].getBoundingClientRect();
		...
		selectPoint: function (pointId) {
			...
			this.annotationBox
				.find('.cw-hotspot-point[data-point-id="' + pointId + '"]')
				.addClass('active');
		},
		...
		showEditModal: function (point) {
			...
			if ($saveButton.length > 0 && $cancelButton.length > 0) {
				...
				$saveButton.on('click.hotspot-save', function (e) {
					...
					currentPoint.linkTarget = $(
						'#cw-hotspot-modal-link-target'
					).val();
					currentPoint.iconName = iconValue || 'plus';
					currentPoint.contentType =
						$('#cw-hotspot-modal-content-type').val() || 'text';
					currentPoint.postId =
						$('#cw-hotspot-modal-post-id').val() || '';
					currentPoint.postType =
						$('#cw-hotspot-modal-post-type').val() || '';
					currentPoint.postTemplate =
						$('#cw-hotspot-modal-post-template').val() || 'default';
					currentPoint.popoverWidth =
						$('#cw-hotspot-modal-popover-width').val() || '';
					currentPoint.wrapperClass =
						$('#cw-hotspot-modal-wrapper-class').val() || '';
			...
			const postsMapExists =
				$postTypeSelect.data('posts-map') !== undefined;
				...
				if (point.postType) {
					...
					if (postsMap && postsMap[postType]) {
						...
						defaultOption.textContent =
							cwHotspotAdmin.i18n.selectPostPlaceholder ||
							'-- Select Post --';
					...
					this.loadTemplatesForPostType(postType).then(() => {
						...
						setTimeout(() => {
							$('#cw-hotspot-modal-post-template').val(
								savedTemplate
							);
						}, 100);
					});
					...
					this.loadPostType(postId).then((postType) => {
						if (postType) {
							...
							if (postsMap && postsMap[postType]) {
								...
								postsMap[postType].posts.forEach(
									function (post) {
										const option =
											document.createElement('option');
										option.value = post.id;
		...
		loadPostType: function (postId) {
			...
			const ajaxurl =
				typeof fetch_vars !== 'undefined' && fetch_vars.ajaxurl
					? fetch_vars.ajaxurl
					: typeof cwHotspotAdmin !== 'undefined' &&
		...
		getOrCreateModal: function () {
			...
			if (modal.length === 0) {
				modal = $('<div>')
					...
					.attr('class', 'cw-hotspot-modal').html(`
				...
				modal.on('click.hotspot-backdrop', function (e) {
					...
					if (
						$(e.target).is('.cw-hotspot-modal') ||
						$(e.target).is('.cw-hotspot-modal-close')
					) {
						modal.removeClass('active');
					}
				});
				...
				modal
					.find('.cw-hotspot-modal-close')
					.off('click.hotspot-close');
				modal
					.find('.cw-hotspot-modal-close')
					.on('click.hotspot-close', function (e) {
						e.preventDefault();
						e.stopPropagation();
						modal.removeClass('active');
						return false;
		...
		initPostSelector: function () {
			...
			if (
				$postTypeSelect.data('posts-map') !== undefined &&
				$postTypeSelect.find('option').length > 1
			) {
			...
			return fetch(ajaxurl, {
				...
				.then(function (result) {
					if (
						...
						Object.keys(postTypesMap)
							.sort()
							.forEach(function (postType) {
								const option = document.createElement('option');
								option.value = postType;
								option.textContent =
									postTypesMap[postType].label;
						...
						$postTypeSelect.off('change').on('change', function () {
							...
							if (selectedType) {
								self.loadTemplatesForPostType(
									...
									if (
										...
										const defaultTemplate = $templateSelect
											.find('option')
											.first()
											.val();
							...
							if (
								...
								// Заполняем постами выбранного типа
								postsMap[selectedType].posts.forEach(
									function (post) {
										const option =
											document.createElement('option');
										option.value = post.id;
			...
			function buildIconList(filter = '') {
				...
				filteredIcons.forEach(function (icon) {
					const $item = $('<div>')
						...
						.html(
							'<span class="cw-icon-item-icon"><i class="' +
								icon.class +
								'"></i></span><span class="cw-icon-item-label">' +
								icon.label +
								'</span>'
						)
						.on('click', function (e) {
		...
		updateIconPreview: function (iconName) {
			...
			const icon = icons.find(function (i) {
				return i.value === iconName;
			});
			if (icon) {
		...
		saveSettings: function () {
			const settings = {
				hotspotButtonStyle:
					$('#hotspot-button-style').val() || 'btn-primary',
