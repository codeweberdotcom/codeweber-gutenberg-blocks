...
export const Banner11 = ({ attributes, isEditor = false, clientId = '' }) => {
	...
	const getSectionClasses = () => {
		const classes = ['wrapper', 'image-wrapper', 'bg-image', 'bg-overlay', 'bg-overlay-400', 'bg-content', 'text-white'];
			...
			<div className="col-lg-5 offset-lg-1">
				...
				>
					{imagesForSwiper.map((image, index) => {
						...
						const showVideoButton = !isEditor && videoUrl && index === 1;

						return (
							<SwiperSlide key={`banner11-slide-${index}-${swiperUniqueKey}`}>
								{showVideoButton && (
									<a 
										href={videoUrl} 
