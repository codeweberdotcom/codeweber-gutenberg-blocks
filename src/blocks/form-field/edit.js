...
const FormFieldEdit = ({ attributes, setAttributes }) => {
	...
	const getDefaultFieldLabel = (type) => {
		...
		return (
			defaultLabels[type] ||
			__('Field Label', 'codeweber-gutenberg-blocks')
		);
	};
	...
	useEffect(() => {
		if (
			...
			apiFetch({
				path: '/wp/v2/legal?per_page=100&orderby=title&order=asc',
			})
				.then((docs) => {
	...
	useEffect(() => {
		if (fieldType === 'consents_block' && consents && consents.length > 0) {
			consents.forEach((consent, index) => {
				...
				if (
					consent.document_id &&
					(!consent.label || consent.label.trim() === '') &&
					!loadingLabels[index]
				) {
	...
	const getDefaultLabel = async (documentId, consentIndex) => {
		...
		if (currentLabel.trim()) {
			if (
				!window.confirm(
					__(
						'Replace existing label text with default text for this document?',
		...
		try {
			...
			console.log(
				'Getting default label for document:',
				documentId,
				'nonce:',
				nonce ? 'present' : 'missing'
			);
			...
			setAttributes((currentAttributes) => {
				...
				if (currentConsents[consentIndex]) {
					...
					if (ajaxResponse?.success && ajaxResponse?.data?.label) {
						...
						const docTitle =
							docResponse?.title?.rendered ||
							docResponse?.title ||
							'';
						const docLink = docResponse?.link || '#';
						const defaultLabel = __(
							'I agree to the {document_title_url}',
							'codeweber-gutenberg-blocks'
						).replace(
							'{document_title_url}',
	...
	const fieldTypes = [
		...
		{
			label: __('Consents block', 'codeweber-gutenberg-blocks'),
			value: 'consents_block',
		},
		{ label: __('Rating', 'codeweber-gutenberg-blocks'), value: 'rating' },
		{
			label: __('Author Role (Position)', 'codeweber-gutenberg-blocks'),
			value: 'author_role',
		},
		{
			label: __('Company', 'codeweber-gutenberg-blocks'),
			value: 'company',
		},
	];
	...
	const widthOptions = [
		{
			label: __('Full Width (col-12)', 'codeweber-gutenberg-blocks'),
			value: 'col-12',
		},
		{
			label: __('Half Width (col-md-6)', 'codeweber-gutenberg-blocks'),
			value: 'col-md-6',
		},
		{
			label: __('Third Width (col-md-4)', 'codeweber-gutenberg-blocks'),
			value: 'col-md-4',
		},
		{
			label: __('Quarter Width (col-md-3)', 'codeweber-gutenberg-blocks'),
			value: 'col-md-3',
		},
	];
	...
	const isInlineButtonSupported =
		inlineButtonSupportedTypes.includes(fieldType);

	// Рендер предпросмотра поля
	const renderFieldPreview = () => {
		const fieldId = `field-${fieldName || 'preview'}`;
		const controlClass = (base) =>
			[base, fieldClass || ''].filter(Boolean).join(' ').trim();
		...
		switch (fieldType) {
				...
				return (
					<div className="testimonial-rating-selector">
						<label className="form-label d-block mb-3">
							{fieldLabel ||
								__('Rating', 'codeweber-gutenberg-blocks')}
							{isRequired && (
								<span className="text-danger"> *</span>
							)}
						</label>
						<div
							className="rating-stars-wrapper d-flex gap-1 align-items-center"
			...
			case 'file':
				...
				if (maxFileSize) {
					fileInfo.push(
						__('Max file size', 'codeweber-gutenberg-blocks') +
							': ' +
							maxFileSize
					);
				}
				if (maxFiles && multiple) {
					fileInfo.push(
						__('Max files', 'codeweber-gutenberg-blocks') +
							': ' +
							maxFiles
					);
				} else if (!multiple) {
					fileInfo.push(
						__('Single file', 'codeweber-gutenberg-blocks')
					);
				}
				if (accept) {
					fileInfo.push(
						__('Accepted types', 'codeweber-gutenberg-blocks') +
							': ' +
							accept
					);
				}
				...
				return (
					<div>
						<label htmlFor={fieldId} className="form-label">
							{fieldLabel ||
								__('File Upload', 'codeweber-gutenberg-blocks')}
							{isRequired && (
								<span className="text-danger"> *</span>
							)}
						</label>
			...
			case 'hidden':
				return (
					<div className="text-muted small">
						{__('Hidden Field', 'codeweber-gutenberg-blocks')}:{' '}
			...
			case 'consents_block':
				return (
					<div className="text-muted small p-3 border rounded bg-light">
						<strong>
							{__('Consents Block', 'codeweber-gutenberg-blocks')}
						</strong>
						<br />
						<small>
							{__(
								'This field outputs consents configured below the form.',
								'codeweber-gutenberg-blocks'
							)}
						</small>
					</div>
			...
			default:
				if (enableInlineButton && isInlineButtonSupported) {
					const previewInlineButtonText =
						inlineButtonText ||
						__('Send', 'codeweber-gutenberg-blocks');
							...
							{tab.name === 'consents' &&
								fieldType === 'consents_block' && (
									<PanelBody>
										...
										>
											{__(
												'Add consent checkboxes to your form. Each consent can have a custom label with placeholders for document links.',
												'codeweber-gutenberg-blocks'
											)}
										</p>
										...
										{(consents || []).map(
											(consent, index) => (
												...
												>
														...
														<TextControl
															...
															help={
																loadingLabels[
																	...
																	: __(
																			'You can use placeholders: {document_url}, {document_title}, {document_title_url}, {form_id}',
																			'codeweber-gutenberg-blocks'
																		)
															}
															disabled={
													...
													<ToggleControl
														label={__(
															'Required (form cannot be submitted without this consent)',
															'codeweber-gutenberg-blocks'
														)}
														checked={
															consent.required ||
										...
										>
											<SelectControl
												...
												onChange={(value) => {
													...
													if (value === 'tel') {
														if (!phoneMask) {
															newAttributes.phoneMask =
																'+7 (___) ___-__-__';
														}
														if (!phoneMaskCaret) {
															newAttributes.phoneMaskCaret =
																'_';
														}
														if (
															!phoneMaskSoftCaret
														) {
															newAttributes.phoneMaskSoftCaret =
																'_';
														}
													}
											...
											<ToggleControl
												...
												help={__(
													'Field will be shown only for non-logged-in users (guests)',
													'codeweber-gutenberg-blocks'
												)}
											/>
											{isInlineButtonSupported && (
												<>
													<ToggleControl
														...
														help={__(
															'Place a submit button inside this field (inline button layout)',
															'codeweber-gutenberg-blocks'
														)}
													/>
													{enableInlineButton && (
														<>
															...
															<TextControl
																...
																help={__(
																	'CSS classes for the inline button',
																	'codeweber-gutenberg-blocks'
																)}
															/>
														</>
										...
										{fieldType === 'file' && (
											...
											>
												<TextControl
													...
													help={__(
														'File types: .pdf,.doc,.docx or image/*',
														'codeweber-gutenberg-blocks'
													)}
												/>
												<ToggleControl
												...
												<TextControl
													...
													help={__(
														'Maximum number of files (0 = unlimited). For single file, set to 1.',
														'codeweber-gutenberg-blocks'
													)}
												/>
												<TextControl
													...
													help={__(
														'Maximum size per file (e.g., 5MB, 500KB)',
														'codeweber-gutenberg-blocks'
													)}
												/>
												<TextControl
													...
													help={__(
														'Maximum total size for all files (e.g., 50MB, 1GB)',
														'codeweber-gutenberg-blocks'
													)}
												/>
											</PanelBody>
							...
							{tab.name === 'settings' && (
								<PanelBody>
									<TextControl
										...
										value={fieldClass || ''}
										onChange={(value) =>
											setAttributes({ fieldClass: value })
										}
										help={__(
											'Optional class added to the form element',
											'codeweber-gutenberg-blocks'
										)}
									/>
									<BlockMetaFields
