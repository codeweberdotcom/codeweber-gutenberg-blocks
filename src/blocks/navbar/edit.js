...
const NavbarEdit = ({ attributes, setAttributes }) => {
	...
	useEffect(() => {
		...
		const params = new URLSearchParams({
			...
			logoHtmlEnabled: attributes.logoHtmlEnabled ? '1' : '0',
			logoHtml: attributes.logoHtml || '',
			headerBackground: attributes.headerBackground || '',
			headerBackgroundStyle: attributes.headerBackgroundStyle || 'solid',
			socialFromTheme: attributes.socialFromTheme ? '1' : '0',
			socialStyleType: attributes.socialStyleType || 'type1',
			socialSize: attributes.socialSize || 'sm',
			socialButtonColor: attributes.socialButtonColor || 'primary',
			socialButtonStyle: attributes.socialButtonStyle || 'solid',
			socialButtonForm: attributes.socialButtonForm || 'circle',
			socialNavClass: attributes.socialNavClass || '',
			socialThemeEnabledSlugs: Array.isArray(attributes.socialThemeEnabledSlugs) ? JSON.stringify(attributes.socialThemeEnabledSlugs) : '',
			mobileMenuElements: Array.isArray(attributes.mobileMenuElements) ? JSON.stringify(attributes.mobileMenuElements) : '[]',
			mobileMenuOffcanvasTheme: attributes.mobileMenuOffcanvasTheme || 'light',
			mobileMenuSocialType: attributes.mobileMenuSocialType || '',
			mobileMenuSocialSize: attributes.mobileMenuSocialSize || '',
			mobileMenuSocialStyle: attributes.mobileMenuSocialStyle || '',
			_t: String(Date.now()),
		});
		apiFetch({
		...
		attributes.logoHtmlEnabled,
		attributes.logoHtml,
		attributes.headerBackground,
		attributes.headerBackgroundStyle,
		attributes.socialFromTheme,
		attributes.socialStyleType,
		attributes.socialSize,
		attributes.socialButtonColor,
		attributes.socialButtonStyle,
		attributes.socialButtonForm,
		attributes.socialNavClass,
		attributes.socialThemeEnabledSlugs,
		attributes.mobileMenuElements,
		attributes.mobileMenuOffcanvasTheme,
		attributes.mobileMenuSocialType,
		attributes.mobileMenuSocialSize,
		attributes.mobileMenuSocialStyle,
	]);
