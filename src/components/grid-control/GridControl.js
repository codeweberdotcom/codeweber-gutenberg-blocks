...
export const GridControl = ({
	...
	const gapClassesString =
		gapClasses.length > 0
			? gapClasses.join(' ')
			: __('No Gap Classes', 'codeweber-gutenberg-blocks');

	// Генерируем классы row-cols для отображения
	const rowColsClasses = getRowColsClasses(attributes, attributePrefix);
	const rowColsClassesString =
		rowColsClasses.length > 0
			? rowColsClasses.join(' ')
			: __('No Row Cols Classes', 'codeweber-gutenberg-blocks');
				...
				<div>
					<div style={{ marginBottom: '16px' }}>
						...
						<ButtonGroup style={{ display: 'flex', width: '100%' }}>
							...
							<Button
								variant={
									gapType === 'x' ? 'primary' : 'secondary'
								}
								onClick={() =>
									setAttributes({
										[`${attributePrefix}GapType`]: 'x',
									})
								}
								style={{ flex: '1 1 auto' }}
							>
							...
							<Button
								variant={
									gapType === 'y' ? 'primary' : 'secondary'
								}
								onClick={() =>
									setAttributes({
										[`${attributePrefix}GapType`]: 'y',
									})
								}
								style={{ flex: '1 1 auto' }}
							>
					...
					{gapType === 'general' && (
						<ResponsiveControl
							{...createBreakpointsConfig({
								...
								tooltip: __(
									'Spacing between grid items (both axes)',
									'codeweber-gutenberg-blocks'
								),
								customOptions: {
									default: [
			...
			{showSpacing && (
				<div>
					<SelectControl
						...
						onChange={(value) =>
							setAttributes({
								[`${attributePrefix}SpacingType`]: value,
							})
						}
						help={__(
							'Choose between padding (inside) or margin (outside)',
							'codeweber-gutenberg-blocks'
						)}
					/>
