...
export const getContainerClassNames = (attrs = {}) => {
	...
	const needsFlex =
		attrs.containerJustifyContent && attrs.containerJustifyContent !== '';
	if (needsFlex) {
		classes.push('d-flex');
	}
...
export const getSpacingClasses = (attrs = {}) => {
	...
	if (spacingXxxl) {
		classes.push(`${prefix}-xxxl-${spacingXxxl}`);
	}
...
export const getWaveConfig = (attrs = {}) => {
	const { waveTopEnabled, waveTopType, waveBottomEnabled, waveBottomType } =
		attrs;

	return {
		hasTopWave: waveTopEnabled && waveTopType && WAVE_SVGS[waveTopType],
		hasBottomWave:
			waveBottomEnabled && waveBottomType && WAVE_SVGS[waveBottomType],
		topType: waveTopType,
