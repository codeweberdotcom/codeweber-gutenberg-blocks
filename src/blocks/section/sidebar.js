...
export const SectionSidebar = ({ attributes, setAttributes }) => {
	...
	useEffect(() => {
		if (backgroundVideoPosterId && backgroundVideoPosterId > 0) {
			wp.apiFetch({ path: `/wp/v2/media/${backgroundVideoPosterId}`, method: 'GET' })
				.then((attachment) => {
					if (attachment?.media_details?.filesize) {
						const bytes = attachment.media_details.filesize;
						setPosterSize(bytes < 1024 * 1024
	...
	const renderVideoPickerWithSize = () => {
		...
		return (
			<>
				...
				{/* Poster image picker — shown only when a video is selected */}
				{backgroundVideoUrl && (
					<div className="mb-3">
						<div className="component-sidebar-title">
							<label>
								{__('Video Poster Image', 'codeweber-gutenberg-blocks')}
							</label>
						</div>
						<MediaUploadCheck>
							<MediaUpload
								onSelect={(media) =>
									setAttributes({
										backgroundVideoPosterId: media.id,
										...
										{backgroundVideoPosterUrl && (
											...
											>
												...
												<div
													...
													title={__('Remove Poster', 'codeweber-gutenberg-blocks')}
												>
	...
	const tabs = [
		{
			...
			title: (
				<TabIcon
					icon={layout}
					label={__('Section', 'codeweber-gutenberg-blocks')}
				/>
			),
		},
		{
			...
			title: (
				<TabIcon
					icon={box}
					label={__('Container', 'codeweber-gutenberg-blocks')}
				/>
			),
		},
		{
		...
		},
		{
			...
			title: (
				<TabIcon
					icon={styles}
					label={__('Dividers', 'codeweber-gutenberg-blocks')}
				/>
			),
		},
		{
						...
						<div style={{ padding: '16px' }}>
							...
							>
								<AngledControl
									...
									onAngledEnabledChange={(value) =>
										setAttributes({ angledEnabled: value })
									}
									onAngledUpperChange={(value) =>
										setAttributes({ angledUpper: value })
									}
									onAngledLowerChange={(value) =>
										setAttributes({ angledLower: value })
									}
								/>
							</PanelBody>
							<PanelBody
							...
							>
								<WavesControl
									...
									onWaveTopEnabledChange={(value) =>
										setAttributes({ waveTopEnabled: value })
									}
									onWaveTopTypeChange={(value) =>
										setAttributes({ waveTopType: value })
									}
									onWaveBottomEnabledChange={(value) =>
										setAttributes({
											waveBottomEnabled: value,
										})
									}
									onWaveBottomTypeChange={(value) =>
										setAttributes({ waveBottomType: value })
									}
								/>
							</PanelBody>
						</div>
