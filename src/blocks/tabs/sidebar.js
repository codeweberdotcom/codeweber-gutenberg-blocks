...
export const TabsSidebar = ({ attributes, setAttributes }) => {
	const { tabStyle, tabRounded, tabAlignment, tabBackground, tabsClass, tabsData, tabsId } = attributes;
			...
			>
				...
				<ButtonGroup>
					{[
						...
						{ label: __('Fanny', 'codeweber-gutenberg-blocks'), value: 'fanny' },
					].map((styleOption) => (
						<Button
				...
				{tabStyle === 'fanny' && (
					<>
						...
						<div className="component-sidebar-title" style={{ marginTop: '16px' }}>
							<label>{__('Tab Rounded', 'codeweber-gutenberg-blocks')}</label>
						</div>
						<ButtonGroup>
							{[
								{ label: __('None', 'codeweber-gutenberg-blocks'), value: '' },
								{ label: __('Rounded', 'codeweber-gutenberg-blocks'), value: 'rounded' },
								{ label: __('Rounded 0', 'codeweber-gutenberg-blocks'), value: 'rounded-0' },
								{ label: __('Rounded Pill', 'codeweber-gutenberg-blocks'), value: 'rounded-pill' },
							].map((roundedOption) => (
								<Button
									key={roundedOption.value}
									isPrimary={tabRounded === roundedOption.value}
									onClick={() => handleRoundedChange(roundedOption.value)}
								>
									{roundedOption.label}
								</Button>
							))}
						...
						<div className="component-sidebar-title" style={{ marginTop: '16px' }}>
							<label>{__('Tab Alignment', 'codeweber-gutenberg-blocks')}</label>
						</div>
						<ButtonGroup>
							{[
								...
								<Button
									...
									isPrimary={tabAlignment === alignmentOption.value}
									onClick={() => handleAlignmentChange(alignmentOption.value)}
								>
						...
						<div style={{ marginTop: '16px' }}>
							<ToggleControl
								label={__('Enable Background', 'codeweber-gutenberg-blocks')}
