...
export const LogoSidebar = ({ attributes, setAttributes }) => {
	...
	const tabs = [
		{ name: 'logo', title: <TabIcon icon={siteLogo} label={__('Logo', 'codeweber-gutenberg-blocks')} /> },
	...
	return (
		...
		>
			{(tab) => (
				<>
					{tab.name === 'logo' && (
						<PanelBody title={__('Logo Settings', 'codeweber-gutenberg-blocks')} initialOpen={true}>
				<SelectControl
					label={__('Logo Type', 'codeweber-gutenberg-blocks')}
					value={logoType}
					options={[
						{ label: __('Both (Light & Dark)', 'codeweber-gutenberg-blocks'), value: 'both' },
					...
					help={__('Select logo variant: both shows light/dark based on theme, light or dark shows specific variant.', 'codeweber-gutenberg-blocks')}
				/>

				<BaseControl
					label={__('Logo Size', 'codeweber-gutenberg-blocks')}
					help={__('Set logo size (e.g., 150px, 200px, 50%, auto). Leave empty for default size.', 'codeweber-gutenberg-blocks')}
				>
					<TextControl
						value={logoSize}
						onChange={(value) => setAttributes({ logoSize: value })}
						placeholder={__('e.g., 150px', 'codeweber-gutenberg-blocks')}
					/>
				</BaseControl>

				<ToggleControl
					label={__('Enable Link', 'codeweber-gutenberg-blocks')}
					help={__('Wrap logo in a link (like in header)', 'codeweber-gutenberg-blocks')}
					checked={enableLink}
				...
				{enableLink && (
					<TextControl
						...
						placeholder={__('e.g., / or https://example.com', 'codeweber-gutenberg-blocks')}
						help={__('Leave empty to use home URL', 'codeweber-gutenberg-blocks')}
					/>
						)}
						</PanelBody>
					)}
