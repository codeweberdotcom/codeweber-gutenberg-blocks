<?php
/**
 * Navbar Block - Extended center logo template (navbar-8).
 * Markup aligned with theme sample: topbar (logo centered) + navbar-collapse-wrapper (social | menu | icons).
 *
 ...
 * @var string $extended_bar_class Second row bar class (e.g. bg-white, bg-dark).
 * @var string $navbar_other_inner_blocks Rendered inner blocks for right column (icons).
...
<header class="<?php echo esc_attr($wrapper_class); ?>"<?php if (!empty($block_id_attr)) { echo ' id="' . esc_attr($block_id_attr) . '"'; } ?>>
	<nav class="navbar navbar-expand-lg extended extended-alt <?php echo esc_attr($nav_class); ?><?php if (!empty($header_bg_class)) { echo ' ' . esc_attr($header_bg_class); } ?>">
		<div class="container flex-lg-column">
			...
			<!-- /.topbar -->
			<div class="navbar-collapse-wrapper <?php echo esc_attr($extended_bar_class); ?> d-flex flex-row align-items-center justify-content-between">
				<div class="navbar-other w-100 d-none d-lg-block"><?php if (!empty($navbar_social_html)) { echo $navbar_social_html; } ?></div>
				<!-- /.navbar-other -->
