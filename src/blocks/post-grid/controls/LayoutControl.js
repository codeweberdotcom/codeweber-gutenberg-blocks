...
const GRID_TYPE_OPTIONS = [
	{
		value: 'classic',
		label: __('Classic grid', 'codeweber-gutenberg-blocks'),
	},
	{
		value: 'columns-grid',
		label: __('Columns grid', 'codeweber-gutenberg-blocks'),
	},
];
...
export const LayoutControl = ({ attributes, setAttributes }) => {
	...
	return (
		<>
			<div className="component-sidebar-title">
				<label>
					{__('Display Mode', 'codeweber-gutenberg-blocks')}
				</label>
			</div>
						...
						<>
							...
							>
								...
								<div style={{ wordBreak: 'break-word' }}>
									{(() => {
										const colClasses =
											getColClassesFromGridColumns(
												attributes
											);
										return colClasses.length > 0
											? colClasses.join(' ')
											: __(
													'No Col Classes',
													'codeweber-gutenberg-blocks'
			...
			{displayMode === 'swiper' && (
				<>
					...
					<RangeControl
						...
						value={swiperSpeed}
						onChange={(value) =>
							setAttributes({ swiperSpeed: value })
						}
						min={100}
						max={3000}
						step={100}
					...
					<ToggleControl
						label={
							<LabelWithTooltip
								...
								tooltip={__(
									'Slides will have auto width based on their content. Overrides items per view settings.',
									'codeweber-gutenberg-blocks'
								)}
							/>
						}
					...
					<RangeControl
						...
						onChange={(value) =>
							setAttributes({ swiperMargin: value.toString() })
						}
						min={0}
						max={100}
						step={5}
					...
					<ToggleControl
						label={
							<LabelWithTooltip
								...
								tooltip={__(
									'Enable continuous loop mode. Slides will loop from last to first seamlessly.',
									'codeweber-gutenberg-blocks'
								)}
							/>
						}
					...
					<ToggleControl
						label={
							<LabelWithTooltip
								...
								tooltip={__(
									'Slider wrapper will adapt its height to the height of the currently active slide.',
									'codeweber-gutenberg-blocks'
								)}
							/>
						}
					...
					<ToggleControl
						label={
							<LabelWithTooltip
								...
								tooltip={__(
									'When enabled, Swiper will be disabled and hide navigation buttons when there are not enough slides.',
									'codeweber-gutenberg-blocks'
								)}
							/>
						}
					...
					<ToggleControl
						label={
							<LabelWithTooltip
								...
								tooltip={__(
									'Swiper will recalculate slides position on window resize (orientation change).',
									'codeweber-gutenberg-blocks'
								)}
							/>
						}
					...
					<ToggleControl
						label={
							<LabelWithTooltip
								...
								tooltip={__(
									'Active slide will be centered, not at the beginning of the slider.',
									'codeweber-gutenberg-blocks'
								)}
							/>
						}
					...
					<ToggleControl
						label={
							<LabelWithTooltip
								...
								tooltip={__(
									'Enable automatic slide change. Slides will automatically transition at specified intervals.',
									'codeweber-gutenberg-blocks'
								)}
							/>
						}
					...
					{swiperAutoplay && (
						<>
							<RangeControl
								...
								onChange={(value) =>
									setAttributes({ swiperAutoplayTime: value })
								}
								min={1}
								max={10000}
								step={1}
					...
					<ToggleControl
						label={
							<LabelWithTooltip
								...
								tooltip={__(
									'Show previous/next navigation arrows to manually change slides.',
									'codeweber-gutenberg-blocks'
								)}
							/>
						}
					...
					<ToggleControl
						label={
							<LabelWithTooltip
								...
								tooltip={__(
									'Show pagination bullets/dots indicator at the bottom of the slider.',
									'codeweber-gutenberg-blocks'
								)}
							/>
						}
					...
					<ToggleControl
						label={
							<LabelWithTooltip
								...
								tooltip={__(
									'Enable touch/mouse drag to change slides by swiping with finger or mouse.',
									'codeweber-gutenberg-blocks'
								)}
							/>
						}
					...
					<div style={{ marginBottom: '12px' }}>
						...
						>
							<Button
								...
								onClick={() =>
									setAttributes({ swiperNavPosition: '' })
								}
								style={{ flex: '1 1 auto' }}
							>
							...
							<Button
								variant={
									swiperNavPosition === 'nav-bottom nav-start'
										? 'primary'
										: 'secondary'
								}
								onClick={() =>
					...
					<TextControl
						...
						onChange={(value) =>
							setAttributes({ swiperWrapperClass: value })
						}
						help={__(
							'Class will be applied to swiper-wrapper element',
							'codeweber-gutenberg-blocks'
						)}
					/>
					...
					<TextControl
						...
						onChange={(value) =>
							setAttributes({ swiperSlideClass: value })
						}
						help={__(
							'Class will be applied to all swiper-slide elements',
							'codeweber-gutenberg-blocks'
						)}
					/>
				</>
